...
const VerifyEmailScreen: React.FC<VerifyEmailScreenProps> = ({ email, onVerified, onBack }) => {
  ...
  const handleVerify = async () => {
    ...
    try {
      if (!supabase) throw new Error("Terminal offline.");
  ...
  const handleResend = async () => {
    if (resendTimer > 0 || !supabase) return;
    try {
    ...
    <div className="min-h-screen bg-black flex flex-col animate-fade-in font-sans selection:bg-thedeal-goldBright selection:text-black">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 pt-32">
        <div className="max-w-md w-full bg-thedeal-card border border-white/5 rounded-[3rem] p-10 md:p-12 text-center shadow-2xl relative">
          ...
          <p className="text-thedeal-gray400 text-sm mb-10">Enviamos um código de 6 dígitos para:<br/><span className="text-white font-bold">{email}</span></p>
          ...
          <div className="flex justify-between gap-2 mb-10">
            {code.map((digit, i) => (
              <input
                ...
                className="w-12 h-16 bg-black border-2 border-thedeal-gray700 rounded-xl text-center text-2xl font-black text-thedeal-gold focus:border-thedeal-gold outline-none"
              />
            ))}
          </div>
          ...
          <button
            ...
            className="w-full bg-thedeal-gold text-black font-black py-5 rounded-2xl flex items-center justify-center gap-3 uppercase text-xs tracking-widest disabled:opacity-30"
          >
          ...
          <div className="mt-8 pt-8 border-t border-white/5 flex flex-col gap-4">
            <button 
              ...
              className="text-[10px] font-black uppercase tracking-widest text-thedeal-gold disabled:text-thedeal-gray700 flex items-center justify-center gap-2 mx-auto"
            >
