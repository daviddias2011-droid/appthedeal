...
interface ClubAlphaGateProps {
    user: User;
}

const ClubAlphaGate: React.FC<ClubAlphaGateProps> = ({ user }) => {
                ...
                <div className="space-y-3">
                    ...
                    <div>
                        <p className="text-sm font-bold text-brand-text mb-1">Avaliação Média: {user.rating?.toFixed(1) || 'N/A'} / {ratingNeeded.toFixed(1)}</p>
                         <div className="w-full bg-brand-gray rounded-full h-2.5 border border-brand-border">
                            <div className="bg-brand-primary h-2 rounded-full" style={{ width: `${Math.min(ratingProgress, 100)}%` }}></div>
                        </div>
                    </div>
                </div>
            </>
    ...
    return (
        <div id="alpha-hub-gate" className="bg-gradient-to-br from-brand-gray to-brand-dark border border-brand-primary/50 rounded-lg p-6 mb-8 shadow-lg shadow-brand-primary/10">
            <div className="flex items-start md:items-center gap-4 mb-3 flex-col md:flex-row">
                ...
                <div>
                    ...
                    <p className="text-brand-text-secondary">
                        Acesso exclusivo para parceiros que entregam performance de forma consistente.
                    </p>
                </div>
            </div>
