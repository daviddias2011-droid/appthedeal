...
import { FireIcon, ClockIcon, ShareIcon } from './Icons';
...
const JobCard: React.FC<JobCardProps> = ({ deal, onViewDetails }) => {
  ...
  return (
    <div className={`bg-brand-gray border border-brand-border rounded-lg p-6 transition-all duration-300 hover:border-brand-primary/50 hover:shadow-lg hover:shadow-brand-primary/10 ${deal.isFlashDeal ? 'border-brand-primary/50' : ''}`}>
      {deal.isFlashDeal && (
        <div className="flex items-center gap-2 text-brand-primary mb-3 font-bold">
      ...
      <div className="flex items-center gap-4 mb-4">
        <img src={deal.brand.logoUrl} alt={`${deal.brand.name} logo`} className="w-12 h-12 rounded-full bg-brand-light-gray" />
        <div>
          <h3 className="text-lg font-bold text-brand-text font-display">{deal.title}</h3>
          <p className="text-sm text-brand-text-secondary">{deal.brand.name}</p>
        </div>
      </div>
      <p className="text-brand-text-secondary mb-4 text-sm">{deal.description}</p>
      <div className="flex justify-between items-center">
        <span className="text-xl font-bold text-brand-primary">{currencyFormatter.format(deal.value)}</span>
        <div className="flex items-center gap-2">
            <div className="relative" ref={shareMenuRef}>
                 <button
                    onClick={() => setShareMenuOpen(!isShareMenuOpen)}
                    className="p-2 rounded-md text-brand-text-secondary hover:bg-brand-light-gray hover:text-white transition-colors"
                    aria-label="Compartilhar deal"
                    >
            ...
            className="bg-brand-light-gray text-brand-primary font-bold py-2 px-6 rounded-md hover:bg-brand-border transition-colors">
            Ver Detalhes
            </button>
        </div>
      </div>
